...
cspray = corespraydf_4body(gcname, potential, mu0, sig0, vesc0, rho0, mgc, rgc, W0)
# Sample for one orbital period
p_orb = p_orb_max * 1000  # Myr
os, ot = cspray.sample_four_body(p_orb, nstar=10, triples=True)
...
r_t = ot.R()  # [kpc]
z_t = ot.z()  # [kpc]
# Would tesc of triples be same as escaper star?
v_esc_t = cspray.vesct  # [km/s]
print("vesc_b is:", v_esc_t)
print("vesc is:", v_esc)
...
ax[0, 0].scatter(ra, dec, marker='o', s=50, edgecolor='black', c='#8d99ae',
                 alpha=0.9)
ax[0, 0].scatter(ra_t[bin_esc], dec_t[bin_esc], marker='o', s=50,
...
ax[0, 1].scatter(r_t[bin_esc], z_t[bin_esc], marker='o', s=50,
                 edgecolor='black', c='#ef233c', alpha=0.9,
...
ax[1, 0].scatter(pmra, pmdec, marker='o', s=50, edgecolor='black', c='#8d99ae',
                 alpha=0.9)
ax[1, 0].scatter(pmra_t[bin_esc], pmdec_t[bin_esc], marker='o', s=50,
                 edgecolor='black', c='#ef233c', alpha=0.9)
...
ax[1, 1].hist(v_esc_t, bins=5, color='#ef233c', density=True)
...
fig.tight_layout()
# fig.suptitle("4 Body Parameter Spaces for M3 Cluster")
plt.savefig("Ritik's M3 plot for 4 bodies.png")
plt.show()
