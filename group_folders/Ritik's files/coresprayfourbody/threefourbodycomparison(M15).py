...
sig0 = 13.1  # Central 1D velocity dispersion [km/s]
vesc0 = 48.9  # Central escape velocity [km/s]
logrho0 = 6.65  # Log of central density [Msol / pc^3]
rho0 = 10.0**6.65  # Core density [Msol / pc^3]
mgc = 5.18e5  # Mass [solar masses]
rgc = 132.13  # Tidal radius of GC, assuming King potential [pc]
rcore = 0.06  # Core radius of GC [pc]
W0 = 15.853325495853458

mmin = 0.2  # Minimum stellar mass in core [Msol]
mmax = 0.7  # Maximum stellar mass in the core [Msol]
alpha = -0.97  # Stellar mass function in the core slope (Salpeter 1955)

potential = MWPotential2014  # Galactic potential model
ro = 2.  # radius of the solar circle (needed for galpy)
...
_, bin_num, _ = plt.hist(mstar, bins=20, color='red', edgecolor='black',
                         alpha=0.4, density=True, label='3body escaper mass dist.')
_ = plt.hist(mstar_four, bins=bin_num, color='blue', edgecolor='black',
             alpha=0.4, density=True, label='4body escaper mass dist.')
plt.xlabel("Masses", fontsize=13)
plt.ylabel("Normalized Counts", fontsize=13)
plt.legend(fontsize=10)
plt.savefig('Comparsion of escaper masses (1000 stars, M15).png')
plt.show()
...
plt.ylabel("Normalized Counts", fontsize=13)
plt.legend(fontsize=10)
plt.savefig('Comparsion of binary and triple masses (1000 stars, M15).png')
plt.show()
...
plt.ylabel("Normalized Counts", fontsize=13)
plt.legend(fontsize=10)
plt.savefig('Comparison of single\'s escape velocity (1000 stars, M15).png')
plt.show()
...
plt.ylabel("Normalized Counts", fontsize=13)
plt.legend(fontsize=10)
plt.savefig('Comparison of binaries and triples escape velocities (1000 stars, M15).png')
plt.show()
